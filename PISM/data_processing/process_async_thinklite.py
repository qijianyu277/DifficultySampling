...
class VisualReasoningAccuracy(ORM):
    def __call__(self,
                 completions,
                 ground_truth,  # ✅ 更明确地命名参数为 ground_truth
                 # answer: List[str] = None,  # 可选 fallback 答案字段
                 **kwargs) -> List[float]:
        rewards = []
        pred = self.extract_answer(completions)
        correct = self.extract_answer(ground_truth)
...
async def query_api(
    ...
    payload = {
        "messages": [
            {
                "role": "system",
                "content": "A conversation between User and Assistant. The user asks a question, and the Assistant solves it. The fianl answer MUST BE inside <answer> </answer> tags, i.e., <answer> answer here </answer>'"
            },
            {
                "role": "user",
                "content": [
...
def is_correct(prediction, reference):
    ...
    match = re.match(r"^\s*\([A-Za-z0-9]\)\s*(.*)$", prediction)
    if match:
        prediction = match.group(1)
    boxed_match = re.search(r'\\boxed\{([^}]+)\}', prediction)
    if boxed_match:
        prediction = boxed_match.group(1).strip()
    print("###################")
    print(f"model_output:{prediction}")
    print(f"ground_truth: {reference}")
...
async def process_sample(
    ...
    async with semaphore:  # 控制并发数量
        try:
            # 验证样本数据
            required_keys = ['id', 'prompt','ground_truth', 'image_path']
            ...
            # evaluator = VisualReasoningAccuracy()
            # is_correct_flag = evaluator(model_output,sample["ground_truth"])
            is_correct_flag = is_correct(model_output, sample["ground_truth"])
            print(f"is_correct:{is_correct_flag}")
...
if __name__ == '__main__':
    # 配置参数
    config = {
        "dataset_path": "/mnt/tenant-home_speed/qjy/dataset_filtered/mcts/MCTS_attention.jsonl",
        "url_pool": ["http://localhost:7802/v1/chat/completions"],
        "num_iterations": 10,  # 迭代次数（原num_generations）
        "seed": 42,
        "output_dir": "/mnt/tenant-home_speed/ywr/Token_mask/results_thinklite_new",
    ...
    for ratio in config["mask_ratios"]:
        ...
        # 如果结果文件已存在，直接加载
        if os.path.exists(result_file) and ratio != 0.0:
